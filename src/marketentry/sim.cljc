...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (COG, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up exclusion-duration-cap-exceeded) ==")
    ...
    (println "== filing/submit eng-4 (declared exclusion duration exceeds Art.146's 5-year cap -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up NIU-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (NIU registration unverified -> HARD hold) ==")
