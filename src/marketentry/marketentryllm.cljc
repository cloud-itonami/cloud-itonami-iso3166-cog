(ns marketentry.marketentryllm
  ...
  the Republic of the Congo (COG) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real BOAMP tender response / ARMP
  filing. Every output is censored downstream by `marketentry.governor`
  before anything touches the SSoT, and `:filing/draft`/`:filing/submit`
    ...
    {:summary    (str subject " 向け提出ドラフト提案"
     ...
     :rationale  (if e
                   (str "jurisdiction=" (:jurisdiction e)
                        " procurement-channel=" (:procurement-channel e))
                   "engagementが見つかりません")
     :cites      (if e [subject] [])
